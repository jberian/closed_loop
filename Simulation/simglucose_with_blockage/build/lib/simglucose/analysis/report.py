...
def ensemble_Insulin(Insulin, Blockage, ax=None):
    # t = BG.index.to_pydatetime()
...
def ensemblePlot(df):
    ...
    df_Blockage = df.unstack(level=0).blockage
    ...
    ax3 = ensemble_Insulin(df_Insulin, df_Blockage, ax=ax3)
    # t = df_CHO.index.to_pydatetime()
    t = pd.to_datetime(df_CHO.index)
    #ax3.plot(t, df_Insulin, label='Dosed insulin')
    #ax3.plot(t, df_Blockage, label='Insulin blockage')
    #ax3.legend()
    ax4.plot(t, df_CHO)
    ...
    ax4.xaxis.set_minor_locator(mdates.AutoDateLocator())
    ...
    ax3.set_ylabel('Insulin (units)')
    ax4.set_ylabel('CHO (g)')
    return fig, ax1, ax2, ax3, ax4

def insulinUsage (df):
    df_Insulin = df.unstack(level=0).insulin * 5 # U/min sampled every 5 minutes
    df_Blockage = df.unstack(level=0).blockage * 5 # U/min sampled every 5 minutes
    df_Insulin['t'] = pd.to_datetime(df_Insulin.index)
    p_TDD = df_Insulin.sum()
    p_TDD.name = 'TDD'
    p_Blockage = df_Blockage.sum()
    p_Blockage.name = 'Blockage'
    mask = (df_Insulin['t'] < datetime.today().replace(hour=12, minute=00, second=0))
    p_TDD_0_to_12 = df_Insulin[mask].sum()
    p_TDD_0_to_12.name = 'TDD_0_to_12'
    mask = (df_Insulin['t'] >= datetime.today().replace(hour=12, minute=00, second=0))
    p_TDD_from_12 = df_Insulin[mask].sum()
    p_TDD_from_12.name = 'TDD_from_12'
    insulinStats = pd.concat([p_TDD, p_TDD_0_to_12, p_TDD_from_12, p_Blockage],axis=1)
    return insulinStats
...
def report(df, save_path=None):
    ...
    fig_ensemble, ax1, ax2, ax3, ax7 = ensemblePlot(df)
    pstats, fig_percent, ax4 = percent_stats(BG)
    insulinStats = insulinUsage(df)
    ...
    results = pd.concat([pstats, ri_mean, insulinStats], axis=1)

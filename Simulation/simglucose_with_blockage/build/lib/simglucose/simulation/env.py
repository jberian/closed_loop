...
class T1DSimEnv(object):
    ...
    def mini_step(self, action):
        ...
        blockage = action.blockage
        insulin = basal + bolus
        CHO = patient_action.meal
        patient_mdl_act = Action(insulin=insulin, CHO=CHO, blockage=blockage)
        ...
        return CHO, insulin, BG, CGM, blockage

    def step(self, action, reward_fun=risk_diff):
        '''
        action is a namedtuple with keys: basal, bolus and blockage
        '''
        CHO = 0.0
        insulin = 0.0
        BG = 0.0
        ...
        for _ in range(int(self.sample_time)):
            ...
            tmp_CHO, tmp_insulin, tmp_BG, tmp_CGM, tmp_blockage = self.mini_step(action)
            CHO += tmp_CHO / self.sample_time
            insulin += tmp_insulin / self.sample_time
            BG += tmp_BG / self.sample_time
            CGM += tmp_CGM / self.sample_time
            blockage += tmp_blockage / self.sample_time
        ...
        self.blockage_hist.append(blockage)
    ...
    def show_history(self):
        ...
        df['blockage'] = pd.Series(self.blockage_hist)

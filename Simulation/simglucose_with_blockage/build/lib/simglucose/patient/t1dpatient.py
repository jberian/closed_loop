...
Action = namedtuple("patient_action", ['CHO', 'insulin','blockage'])
...
class T1DPatient(Patient):
    ...
    def model(t, x, action, params, last_Qsto, last_foodtaken):
        ...
        final_action_insulin = action.insulin - action.blockage
        if (final_action_insulin < 0.0):
            final_action_insulin = 0.0
        #insulin = action.insulin * 6000 / params.BW  # U/min -> pmol/kg/min
        insulin = final_action_insulin * 6000 / params.BW # U/min -> pmol/kg/min
    ...
    def reset(self):
        ...
        self._last_action = Action(CHO=0, insulin=0, blockage=0.0)
        self.is_eating = False
        self.planned_meal = 0
...
if __name__ == '__main__':
    ...
    while p.t < 1000:
        ...
        # if p.t == 150:
        #     ins = 80.0 / 12.0 + basal
        act = Action(insulin=ins, CHO=carb, blockage=0.0)
        t.append(p.t)
        CHO.append(act.CHO)

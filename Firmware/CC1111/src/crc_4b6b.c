...
unsigned char distanceBetween (char symbol1, char symbol2)
{
  char xorRes;
  unsigned char numDifferences, i;
  
  numDifferences = 0;
  xorRes = symbol1 ^ symbol2;
  for (i=0; i<6; i++) {
    if (xorRes & 0x01) numDifferences++;
    xorRes = xorRes >> 1;
  }
...
char pickMinErrorSymbol (char symbol) {
  unsigned int minDistance, currentDistance;
  char i, chosenSymbol;
  char pattern[16] = {0x15, 0x31, 0x32, 0x23, 0x34, 0x25, 0x26, 0x16,
                      0x1A, 0x19, 0x2A, 0x0B, 0x2C, 0x0D, 0x0E, 0x1C};
  
  minDistance = 7;
  chosenSymbol = 0x00;
  
  for (i=0; i<0x10; i++) {
    currentDistance = distanceBetween(symbol,pattern[i]);
...
char decode4b6bsymbol (char inSymbol, char *outSymbol)
{
  ...
  default:   {*outSymbol = pickMinErrorSymbol(inSymbol); return(1); }
  }
}
...
{
  int i,j;
  for (j=0; j<time*16; j++) for (i=0; i<0xFFFF; i++) asm("nop");
}